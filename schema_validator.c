...
#define MAX_CONSTRAINTS                                                        \
	8 // maximum number of constraints when validating keyword values(or children values)
char *json_keywords_char[] = {
	[json_keyword_multipleOf] = "multipleOf",
...
char *json_values[] = {
	[json_format_date_time] = "date-time",
	...
	"object",
	"array",
	"string", // 7 primitive types of json-c + "number" is allowed in v4 draft
	NULL
};

char *json_format_regex[] = {
...
int json_get_keyword_id(const char *keyword)
{
	int i, size = sizeof(json_keywords_char) /
		      sizeof(json_keywords_char[0]);
	for (i = 0; i < size; i++)
...
int json_get_key_type(json_object *obj)
{
	struct lh_entry *type_entry =
		lh_table_lookup_entry(json_object_get_object(obj), "type");
	...
	if (type_entry != NULL) {
		...
		for (i = 0; json_types_id[i].skey; i++) {
			if (strcmp(json_types_id[i].skey,
				   json_object_get_string(
					   (json_object *)type_entry->v)) ==
			    0) {
...
int json_validate_array_items_uniqueness(json_object *jobj, const int type)
{
	if (json_object_get_type(jobj) != json_type_array) {
		json_printf_colored(ANSI_COLOR_RED, "type must be array.");
		return 0;
	}
	int arraylen = json_object_array_length(
		jobj); /*Getting the length of the array*/
	int i;
	//sort the array
	...
	for (i = 0; i < arraylen - 1; i++) {
		...
		if (type != -1)
			if (((int)json_object_get_type(jvalue)) != type)
				return 0; // not valid type
		if (strcmp(json_object_to_json_string(jvalue),
			   json_object_to_json_string(jvalue1)) == 0)
			return 0; //not unique
	}
	return 1;
}

int json_validate_array_items(json_object *jobj)
{
	if (json_object_get_type(jobj) != json_type_array) {
		json_printf_colored(ANSI_COLOR_RED, "type must be array.");
		return 0;
	...
	for (i = 0; i < arraylen; i++) {
		jvalue = json_object_array_get_idx(
			jobj, i); /*Getting the array element at position i*/
...
int json_is_keyword(const char *keyword)
{
	int array_size =
		sizeof(json_keywords_char) / sizeof(json_keywords_char[0]);
	int i;
...
void json_split_str(const int keyword_position, int *return_array,
		    const int str_pos)
{
...
int json_validate_regex(const char *regex)
{
...
int json_validate_parent_type(const char *type, const int keyword_position)
{
	...
	for (i = 0; json_types_id[i].skey; i++) {
		if (strcmp(type, json_types_id[i].skey) == 0)
			if (json_types_id[i].ikey == parent_type[0] ||
			    json_types_id[i].ikey == parent_type[1])
				return 1;
	}
	return 0;
}

static int json_validate_value_regex(const char *key, json_object *value)
{
	...
	if ((err = regcomp(&pattern, regex, REG_EXTENDED | REG_NOSUB))) {
		...
		json_printf_colored(ANSI_COLOR_RED,
				    "invalid pattern with error: %s", buffer);
		return 0;
	}
	return 1;
}

int json_validate_type(const int type, const int keyword_position)
{
...
int json_check_dependencies(const int keyword_position, lh_table *parent)
{
	int dep = atoi(
		json_keywords_constraints[keyword_position]
					 [3]); // position of the required keyword
	if (dep > -1) {
		char *dep_keyword = json_keywords_constraints
			[dep][0]; // the keyword our keyword depends on
...
int json_check_allowed_values(const int keyword_position,
			      struct json_object *value,
			      struct lh_table *parent, const int items_values)
{
	int allowed_values[MAX_CONSTRAINTS], i, res = 1;
	if (items_values == 1)
		json_split_str(keyword_position, (int *)&allowed_values,
			       5); // get allowed children values ID
	else
		json_split_str(keyword_position, (int *)&allowed_values,
			       4); // get allowed values ID
	...
	for (i = 0; i < MAX_CONSTRAINTS; i++) {
		...
		case json_array_of_unique_string_items_at_least_one_item:
			if (json_object_get_type(value) == json_type_array) {
				if (json_object_array_length(value) < 1) {
					json_printf_colored(
						ANSI_COLOR_RED,
						"at least one item must be present in the array");
					res = 0;
				}
				res = json_validate_array_items_uniqueness(
					value, 6);
			}
			break;
		case json_valid_regex:
			if (json_object_get_type(value) == json_type_string)
				res = json_validate_regex(
		...
		case json_array_of_unique_items_at_least_one_item:
			if (json_object_get_type(value) == json_type_array) {
				...
				res = json_validate_array_items_uniqueness(
					value,
					-1); // no specific type is required
			}
			break;
...
int json_validate_keyword(struct lh_entry *keyword, struct lh_table *parent)
{
	...
	if ((keyword_position = json_is_keyword(keyword_name)) != -1) {
		char *parent_type_value =
			"object"; //assume its an object from the beginning
		struct lh_entry *parent_type =
			lh_table_lookup_entry(parent, "type");
		if (parent_type == NULL) {
			json_printf_colored(
				ANSI_COLOR_YELLOW,
				"---parent type not found assuming its an object");
		} else {
			struct json_object *parent_type_object =
				(struct json_object *)parent_type->v;
			//if the type is an array with more than an element, exit with an error.
			if (json_object_get_type(parent_type_object) ==
			    json_type_array) {
				if (json_object_array_length(
					    parent_type_object) > 1) {
					json_printf_colored(
						ANSI_COLOR_RED,
						"array type of more than one item is not allowed");
					return 0;
				}
				struct json_object *item_value =
					json_object_array_get_idx(
						parent_type_object, 0);
				//item should be a string
				if (json_object_get_type(item_value) !=
				    json_type_string) {
					json_printf_colored(
						ANSI_COLOR_RED,
						"type: array item should be a string");
					return 0;
				}
				parent_type_value =
					(char *)json_object_get_string(
						item_value);
			} else if (json_object_get_type(parent_type_object) ==
				   json_type_string) {
				parent_type_value =
					(char *)json_object_get_string(
						parent_type_object);
				...
				json_printf_colored(
					ANSI_COLOR_RED,
					"type: value type should be either a string or an array");
				return 0;
			}
			//check parent type is valid
			if (json_validate_parent_type(parent_type_value,
						      keyword_position) != 1) {
				json_printf_colored(ANSI_COLOR_RED,
						    "Invalid parent type");
				return 0;
		...
		/* validate values */
		if (json_check_allowed_values(keyword_position,
					      (struct json_object *)keyword->v,
					      parent, 0) != 1) {
			json_printf_colored(ANSI_COLOR_RED, "bad values");
			return 0;
		}
		if (json_object_is_type((struct json_object *)keyword->v,
					json_type_object)) {
			...
			lh_foreach(json_object_get_object(
				   ...
				   child)
			{
				if (json_check_allowed_values(
					    keyword_position,
					    (struct json_object *)child->v,
...
static int json_validate_object(json_object *jobj, const int last_pos)
{
	...
	while (entry != NULL) {
		...
		if (result != 1)
			res = res &&
			      0; // schema is not valid, but keep checking to get all errors.
		if (type == json_type_object)
...
int json_validate_schema(struct json_object *schema)
{
	int res;
	if (!schema)
		return 0;
...
int json_validate_schema_from_file(const char *filename)
{
