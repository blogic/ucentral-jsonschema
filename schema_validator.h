 ...
 */
void json_split_str(int keyword_position, int *allowed_types, int str_pos);
 ...
 */
int json_validate_type(int type, int keyword_position);

/**
 * @brief Validates the dependencies of a keyword
 */
int json_check_dependencies(int keyword_position, lh_table *parent);

/**
 * @brief Validates the supplied value of a keyword
 */
int json_check_allowed_values(int keyword_position, struct json_object *value,
			      struct lh_table *parent, int items_values);

/**
 * @brief Validates a keyword, by checking the provided keyword against the rules of the V4 draft. check schema_rules.txt file
 */
int json_validate_keyword(struct lh_entry *keyword,
			  struct lh_table *parent_type);
