...
static int json_compare_num(double num1, double num2, int strict)
{
	if (strict == 1)
...
static struct json_object *copy_object(struct json_object *obj)
{
...
struct lh_entry *json_get_keyword_entry(const lh_table *table,
					const char *instance_key)
{
	...
	lh_foreach(table, entry)
	{
		...
		struct json_object *entry_object =
		...
		if (json_object_get_type(entry_object) == json_type_object) {
			...
			if ((temp_entry = json_get_keyword_entry(
				     (const lh_table *)json_object_get_object(
					     entry_object),
				     instance_key)) != NULL)
				return temp_entry;
		}
...
static inline int json_handle_multipleOf_keyword(double instance_value,
						 double schema_value)
{
...
static inline int json_handle_maximum_keyword(struct lh_entry *excl_entry,
					      int instance_value,
					      int schema_value)
{
	int excl = (excl_entry != NULL &&
		    json_object_get_boolean((json_object *)excl_entry->v)) ?
			   1 :
			   0;
	int return_error =
		(excl == 1) ?
			json_exclusiveMaximum_error :
			json_maximum_error; // which error to return in case of an error
	return (json_compare_num(instance_value, schema_value, excl) - excl <=
		0) ?
		       1 :
		       return_error;
}

static inline int json_handle_minimum_keyword(struct lh_entry *excl_entry,
					      int instance_value,
					      int schema_value)
{
	int excl = (excl_entry != NULL &&
	...
	int return_error =
		(excl == 1) ?
			...
			json_minimum_error; // which error to return in case of an error
	return (json_compare_num(instance_value, schema_value, excl) - excl >=
		0) ?
		       1 :
		       return_error;
...
int json_validate_numeric_keywords(struct jsonschema_object instance_object)
{
	int res = 1;
	json_object *instance_entry = instance_object.instance;
	json_object *schema_entry = instance_object.instance_schema;
	struct lh_table *key_table = json_object_get_object(
		schema_entry); /* the schema of the key we are validating */
	struct lh_entry *keyword_entry;
	lh_foreach(key_table, keyword_entry)
	{
		const char *keyword = (const char *)keyword_entry->k;
		int keyword_id =
			json_get_keyword_id(keyword); /* get the keyword id */
		struct json_object *schema_value =
			(struct json_object *)keyword_entry->v;

		/* check if its a valid type */
		if (json_object_get_type(instance_entry) != json_type_int &&
		    json_object_get_type(instance_entry) != json_type_double) {
			json_add_error(json_invalid_value_error,
				       instance_object.key,
				       instance_object.object_pos, NULL);
			return 0;
		}
		json_printf_colored(ANSI_COLOR_BLUE, "\t KEYWORD %s %d ",
				    keyword, instance_object.object_pos);
		/* validate the instance against all present keywords */
		switch (keyword_id) {
		case json_keyword_multipleOf:
			res = json_handle_multipleOf_keyword(
				json_object_get_double(instance_entry),
				json_object_get_double(schema_value));
			if (res != 1)
				json_add_error(res, instance_object.key,
					       instance_object.object_pos,
					       NULL);
			break;
		case json_keyword_maximum: /* exclusiveMaximum tells wether its a strict comparison or not */
			res = json_handle_maximum_keyword(
				lh_table_lookup_entry(key_table,
						      "exclusiveMaximum"),
				json_object_get_int(instance_entry),
				json_object_get_int(schema_value));
			if (res != 1)
				json_add_error(res, instance_object.key,
					       instance_object.object_pos,
					       NULL);
		...
		case json_keyword_minimum: /* same for exclusiveMinimum */
			res = json_handle_minimum_keyword(
				lh_table_lookup_entry(key_table,
						      "exclusiveMinimum"),
				json_object_get_int(instance_entry),
				json_object_get_int(schema_value));
...
static inline int json_handle_pattern_keyword(const char *pattern_value,
					      const char *instance_value)
{
	regex_t pattern;
...
int json_validate_string_keywords(struct jsonschema_object instance_object)
{
	int res = 1;
	json_object *instance_entry = instance_object.instance;
	json_object *schema_entry = instance_object.instance_schema;
	...
	lh_foreach(key_table, keyword_entry)
	{
		...
		case json_keyword_pattern:
			res = json_handle_pattern_keyword(
				json_object_get_string(schema_value),
				json_object_get_string(instance_entry));
			if (res != 1)
				json_add_error(res, instance_object.key,
					       instance_object.object_pos,
					       NULL);
...
static inline int
json_handle_additionalItems_keyword(struct json_object *items,
				    struct json_object *instance_entry,
				    struct json_object *schema_value)
{
	/* we need validation only when additionalItems is false and items is an array */
	if (json_object_is_type(schema_value, json_type_boolean) &&
	    !json_object_get_boolean(schema_value) &&
	    json_object_is_type(items, json_type_array)) {
		return (json_compare_num(
				json_object_array_length(items),
				json_object_array_length(instance_entry),
				1) < 0) ?
...
int json_validate_array_keywords(struct jsonschema_object instance_object)
{
	int res = 1;
	json_object *instance_entry = instance_object.instance;
	json_object *schema_entry = instance_object.instance_schema;
	...
	lh_foreach(key_table, keyword_entry)
	{
		...
		case json_keyword_additionalItems:
			res = json_handle_additionalItems_keyword(
				(struct json_object *)json_get_keyword_entry(
					key_table, "items")
					->v,
				instance_entry, schema_value);
			if (res != 1)
				json_add_error(res, instance_object.key,
					       instance_object.object_pos,
					       NULL);
		...
		case json_keyword_uniqueItems:
			res = (json_object_get_boolean(schema_value) &&
			       json_validate_array_items_uniqueness(
				       instance_entry, -1) != 1) ?
...
static inline int json_handle_required_keyword(struct json_object *schema_value,
					       struct lh_table *instance_table)
{
	unsigned int i;
	for (i = 0; i < json_object_array_length(schema_value); i++) {
		const char *required_key = json_object_get_string(
			json_object_array_get_idx(schema_value, i));
		if (lh_table_lookup_entry(instance_table, required_key) ==
		    NULL) {
...
static int
json_handle_additionalProperties_keyword(struct json_object *schema_value,
					 struct lh_table *instance_table,
					 struct lh_table *key_table)
{
	int num_founds = 0; // keep track of valid properties
	if (json_object_get_type(schema_value) == json_type_boolean &&
	    json_object_get_boolean(schema_value) == 0) {
		...
		struct json_object *properties =
			(struct json_object *)lh_table_lookup_entry(
				key_table, "properties")
				->v;
		struct json_object *patternProperties =
			(struct json_object *)lh_table_lookup_entry(
				key_table, "patternProperties")
				->v;
		...
		{
			if (properties != NULL) {
				struct lh_table *properties_table =
					json_object_get_object(properties);
				if (lh_table_lookup_entry(
					    properties_table,
					    (char *)instance_temp_entry->k) !=
				    NULL) {
					num_founds++;
			...
			if (patternProperties != NULL) {
				...
				memset(&pattern, 0, sizeof(regex_t));
				struct lh_entry *temp_entry;
				lh_foreach(json_object_get_object(
						   patternProperties),
					   temp_entry)
				{
					...
					if (regcomp(&pattern,
						...
						json_printf_colored(
							ANSI_COLOR_RED,
							"failed compiling the regex.");
						return json_additionalProperties_error;
					}
					if (regexec(&pattern,
						    (char *)instance_temp_entry
							    ->k,
						    0, NULL, 0) == 0) {
...
static int
json_handle_dependencies_keyword(struct jsonschema_object instance_object,
				 struct json_object *schema_value,
				 struct lh_table *key_table)
{
	...
	lh_foreach(schema_table, value)
	{
		...
		if (json_object_is_type((struct json_object *)value->v,
					json_type_array)) {
			/* check if the property exist in the instance */
			if (lh_table_lookup_entry(instance_table,
						  (char *)value->k) != NULL) {
				...
				for (i = 0;
				   ...
				   (struct json_object *)value->v,
				   json_type_object)) { /* its a schema dependency */
			struct lh_table *dependencies_table =
			...
			lh_foreach(dependencies_table, dependency)
			{
				...
				struct lh_entry *key = lh_table_lookup_entry(
					key_table, (char *)dependency->k);
				if (key != NULL) {
					lh_foreach(
						json_object_get_object(
							(struct json_object *)
					...
					lh_table_insert(key_table,
							dependency->k,
							dependency->v);
			}
			/* temporarly remove this keyword so we dont enter in an endless loop */
			struct lh_entry *dependencies = lh_table_lookup_entry(
				key_table, "dependencies");
			...
			lh_foreach(dependencies_table, dependency)
			{
				...
				if (key != NULL) {
					...
					struct lh_entry *head = lh_table_lookup_entry(
						json_object_get_object(
							(struct json_object *)
								key->v),
						json_object_get_object(
...
int json_validate_object_keywords(struct jsonschema_object instance_object)
{
	int res = 1;
	json_object *instance_entry = instance_object.instance;
	json_object *schema_entry = instance_object.instance_schema;
	...
	struct lh_entry *keyword_entry;
	struct lh_table *instance_table =
		json_object_get_object(instance_entry);
	...
	lh_foreach(key_table, keyword_entry)
	{
		...
		case json_keyword_maxProperties:
			res = json_compare_num(
				      lh_table_length(instance_table),
				      json_object_get_int(schema_value),
				      1) > 0 ?
		...
		case json_keyword_required:
			res = json_handle_required_keyword(schema_value,
							   instance_table);
			if (res != 1)
				json_add_error(res, instance_object.key,
					       instance_object.object_pos,
		...
		case json_keyword_additionalProperties:
			res = json_handle_additionalProperties_keyword(
				schema_value, instance_table, key_table);
			if (res != 1)
				json_add_error(res, instance_object.key,
					       instance_object.object_pos,
					       NULL);
		...
		case json_keyword_dependencies:
			res = json_handle_dependencies_keyword(
				instance_object, schema_value, key_table);
			if (res != 1)
				json_add_error(res, instance_object.key,
					       instance_object.object_pos,
					       NULL);
...
static inline int json_handle_enum_keyword(struct json_object *schema_value,
					   struct json_object *instance_entry)
{
	unsigned int i;
	for (i = 0; i < json_object_array_length(schema_value); i++) {
		const char *enum_val = json_object_to_json_string(
			json_object_array_get_idx(schema_value, i));
		if (strcmp(json_object_to_json_string(instance_entry),
			   enum_val) == 0) {
			return 1;
		}
	}
 ...
 */
static int json_get_instance_schema_from_Of_keywords(
	struct jsonschema_object instance_object,
	struct lh_entry *keyword_entry, struct json_object *temp_schema)
{
	struct lh_table *temp_schema_table =
		json_object_get_object(temp_schema);
...
static inline int
json_handle_allOf_keyword(struct jsonschema_object instance_object,
			  struct lh_entry *keyword_entry,
			  json_object *schema_value)
{
	unsigned int i;
	for (i = 0; i < json_object_array_length(schema_value); i++) {
		json_printf_colored(ANSI_COLOR_CYAN,
				    "Validating allOf schema #%d", i);
		int res = json_get_instance_schema_from_Of_keywords(
			instance_object, keyword_entry,
			json_object_array_get_idx(schema_value, i));
		if (res != 1)
...
static inline int
json_handle_anyOf_keyword(struct jsonschema_object instance_object,
			  struct lh_entry *keyword_entry,
			  json_object *schema_value)
{
	unsigned int i;
...
static inline int
json_handle_oneOf_keyword(struct jsonschema_object instance_object,
			  struct lh_entry *keyword_entry,
			  json_object *schema_value)
{
...
static inline int
json_handle_not_keyword(struct jsonschema_object instance_object,
			struct lh_entry *keyword_entry,
			struct json_object *schema_value)
{
	int res = json_get_instance_schema_from_Of_keywords(
		instance_object, keyword_entry, schema_value);
	return (res == 1 ? json_not_error : 1);
}

int json_validate_anytype_keywords(struct jsonschema_object instance_object)
{
	int res = 1;
	json_object *instance_entry = instance_object.instance;
	json_object *schema_entry = instance_object.instance_schema;
	...
	lh_foreach_safe(key_table, keyword_entry, tmp_keyword_entry)
	{
		...
		case json_keyword_enum:
			res = json_handle_enum_keyword(schema_value,
						       instance_entry);
			if (res != 1)
				json_add_error(res, instance_object.key,
					       instance_object.object_pos,
...
int json_validate_instance_keywords(struct jsonschema_object instance_object,
				    int key_type)
{
...
int json_validate_array_instance(struct jsonschema_object instance_object)
{
	...
	if (items_schema != NULL &&
		...
		unsigned int i;
		for (i = 0;
		     i < json_object_array_length(instance_object.instance);
		     i++) {
			json_object *obj = json_object_array_get_idx(
				instance_object.instance, i);
			struct json_object *items_schema_copy =
				copy_object(items_schema);
			struct jsonschema_object tmp_instance_object;
			tmp_instance_object.key = "instance object";
			tmp_instance_object.object_pos =
				i + 1; /* i starts from 0 */
...
int json_validate_object_instance(struct jsonschema_object instance_object)
{
	//an instance can be either an object or an array of objects.
	//if its an array of objects, validate each one.
	struct lh_table *instance_table =
		json_object_get_object(instance_object.instance);
	struct lh_table *schema_table =
		json_object_get_object(instance_object.instance_schema);
	...
	lh_foreach(instance_table, instance_entry)
	{
		...
		struct json_object *instance_value =
			(struct json_object *)instance_entry->v;
		//lookup the key in the schema
		struct lh_entry *schema_entry =
			lh_table_lookup_entry(schema_table, instance_key);
		//lookup the key in the schema
		if (schema_entry == NULL) {
			//look for the properties keyword
			struct lh_entry *properties_entry;
			properties_entry = lh_table_lookup_entry(schema_table,
								 "properties");
			//look for the key inside the properties object
			if (properties_entry != NULL) {
				schema_entry = lh_table_lookup_entry(
					json_object_get_object(
						(json_object *)
							properties_entry->v),
					instance_key);
			...
			if (schema_entry == NULL) {
				...
				*/
				if ((lh_table_lookup_entry(
				     ...
				     lh_table_lookup_entry(schema_table,
							   "anyOf") == NULL) &&
				    (lh_table_lookup_entry(schema_table,
							   "allOf") == NULL &&
				     lh_table_lookup_entry(schema_table,
		...
		struct json_object *schema_value =
			(struct json_object *)schema_entry->v;
		...
		if (json_object_get_type(instance_value) == json_type_object){
			...
			tmp_res = json_validate_anytype_keywords(
					  instance_object) &&
				  tmp_res;
			tmp_res = json_validate_instance_keywords(
					  instance_object, key_type) &&
				  tmp_res;
		}
	}
	return tmp_res;
...
int json_validate_instance(struct json_object *instance,
			   struct json_object *schema)
{
	...
	if (json_validate_schema(schema) == 1) {
		json_printf_colored(ANSI_COLOR_YELLOW,
		...
		// retrieve errors
		struct json_error *errors =
			malloc((256 * sizeof(struct json_error)));
		int num_errors;
		errors = json_add_error(0, NULL, -1, &num_errors);
		if (num_errors > 0) {
			json_printf_colored(
				ANSI_COLOR_RED,
				"\nInvalid JSON file! %d error(s) found",
				num_errors);
			int i;
...
int json_validate_instance_from_file(const char *instance_path,
				     const char *schema_path)
{
	...
	if (schema == NULL) {
		json_printf_colored("could not load schema from file.",
				    ANSI_COLOR_RED);
		return -1;
	}
	if (instance == NULL) {
		json_printf_colored("could not load instance from file.",
				    ANSI_COLOR_RED);
		return -1;
	}
