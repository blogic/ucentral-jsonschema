...
void json_printf_colored(char *color, char *message, ...)
{
	printf("%s", color);
	va_list vl;
	va_start(vl, message);
...
struct json_error *json_add_error(int error_value, char *key, int obj_pos,
				  int *size)
{
	...
	if (error_value < 0 && key != NULL && obj_pos >= 0) {
		...
		errors[i].error_message =
			json_error_messages[1000 + error_value];
